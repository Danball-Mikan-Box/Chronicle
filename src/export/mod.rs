...
pub fn export_site_zip(project: &Project, output_path: &Path) -> Result<(), String> {
    ...
    let description = &project.settings.description;
    let desc_html = if description.is_empty() { "".to_string() } else { format!("<div class=\"project-desc\">{}</div>", description.replace('\n', "<br>\n")) };
    // Build index page
    let index_html = format!(
        ...
        project.name, css, project.name, project.name, author_html, desc_html, toc
    );
